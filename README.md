...
- Сессии хранятся в Redis, cookie настроены как `httponly`, `same_site: :lax`, `secure` включается в production, если не переопределен `SESSION_COOKIE_SECURE` для HTTP-only test VM.
...
| `APP_PORT` | Host-порт для Docker Compose, по умолчанию `3000`. Для VM можно указать `80`. |
| `SESSION_COOKIE_SECURE` | `true` для HTTPS production, `false` для HTTP-only test VM. |
...
- Compose запускает production-like контейнер Rails на `http://localhost:${APP_PORT:-3000}`, PostgreSQL и Redis с отдельными volumes.
...
- Сессии хранятся в Redis, cookie `httponly`, `same_site: :lax`, `secure` включается в production, если не переопределен `SESSION_COOKIE_SECURE` для HTTP-only test VM.
